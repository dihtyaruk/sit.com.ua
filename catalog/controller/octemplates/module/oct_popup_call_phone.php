...
class ControllerOCTemplatesModuleOctPopupCallPhone extends Controller {
    ...
    protected function validate() {
        ...
        if (isset($this->request->post['time']) && (isset($oct_popup_call_phone_data['time']) && $oct_popup_call_phone_data['time'] == 2) && empty($this->request->post['time'])) {
            $this->error['time'] = $this->language->get('error_time');
        } elseif ((isset($oct_popup_call_phone_data['time']) && $oct_popup_call_phone_data['time'] == 2) && empty($this->request->post['time'])) {
			$this->error['time'] = $this->language->get('error_time');
        }
